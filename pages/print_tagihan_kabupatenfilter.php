...
$pdf->ln(1);
$pdf->SetFont('Arial','B',12);
$pdf->Cell(19.5,0.7,"Laporan Tagihan Kabupaten Pajak Bumi dan Bangunan",0,10,'C');
...
$pdf->Cell(2.7, 0.8, 'Kecamatan', 1, 0, 'C');
$pdf->Cell(1.5, 0.8, 'Baku', 1, 0, 'C');
$pdf->Cell(2.5, 0.8, 'Jumlah', 1, 0, 'C');
$pdf->Cell(2, 0.8, 'Realisasi', 1, 0, 'C');
$pdf->Cell(2.7, 0.8, 'Jumlah Realisasi', 1, 0, 'C');
$pdf->Cell(1.5, 0.8, 'Selisih', 1, 0, 'C');
$pdf->Cell(2.5, 0.8, 'Jumlah Selisih', 1, 1, 'C');
$pdf->SetFont('Arial','',10);
$no=1;
for($i=$id;$i<=$id2;$i++){
	...
	$pdf->Cell(2.7, 0.8, $lihat['kecamatan'], 1, 0,'C');
	$pdf->Cell(1.5, 0.8, $lihat['op_baku'], 1, 0,'C');
	$pdf->Cell(2.5, 0.8, $lihat['op_jumlah'], 1, 0,'C');
	$pdf->Cell(2, 0.8, $lihat['op_realisasi'],1, 0, 'C');
	$pdf->Cell(2.7, 0.8, $lihat['jml_realisasi'], 1, 0,'C');
	$pdf->Cell(1.5, 0.8, $lihat['op_selisih'],1, 0, 'C');
	$pdf->Cell(2.5, 0.8, $lihat['jml_selisih'], 1, 1,'C');

	$no++;
}
}

$pdf->Output("Tagihan Kabupaten Filter.pdf","I");
